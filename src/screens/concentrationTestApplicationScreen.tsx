...
const ConcentrationTestApplicationScreen = ({ navigation, route }: ConcentrationTestApplicationScreenProps) => {
  ...
  // Mantém refs sincronizadas com states
  ...
  // Ref para o handleRoundEnd sempre atualizado (usado no intervalo)
  ...
  const handleNext = () => {
    ...
    const label = isLastRound ? 'Concluir' : 'Próximo';
    Alert.alert(
      `${label} Rodada`,
      isLastRound
        ? 'Deseja concluir o teste? Os resultados serão salvos.'
        : `Deseja avançar para a rodada ${currentRound + 1}?`,
      [
        { text: 'Cancelar', style: 'cancel' },
        { text: label, onPress: handleRoundEnd },
      ],
    );
  };
